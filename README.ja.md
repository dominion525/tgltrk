...
ユーザー情報・プロジェクト・タグの一覧は API 呼び出しを削減するため 72 時間キャッシュされます。Toggl Track API にはレートリミットがあり、キャッシュによって通常利用時にリミットに達することを防ぎます。
...
- **タイムエントリの一括操作**: 一覧表示・取得はサポートしていますが、一括削除などの操作は未対応です
- **レポート機能**: Toggl Track のレポートエンドポイント（概要・詳細・週次）は未対応です
- **ワークスペース管理**: ワークスペースの作成・変更はできません。`--workspace` は既存のワークスペースの選択のみです
- **有料プラン機能**: 有料プラン限定の機能（プロジェクトテンプレート、時間見積もり、必須フィールドなど）は未対応です
- **レートリミット**: Toggl Track API にはレートリミットがあります。CLI はユーザー情報・プロジェクト・タグを 72 時間キャッシュして API 呼び出しを最小限に抑えています。レートリミットに達した場合はリセットを待ってから再試行してください
